        ...
        if (error) {
          switch (error.status) {
            case 400:
              ...
              break;
            case 401:
              this.toastr.error('Non Autorizzato', error.status.toString())
              break;
            case 404:
              this.router.navigateByUrl('/not-found');
              this.toastr.error('Non Trovato', error.status.toString())
              break;
            case 500:
              const navigationExtras: NavigationExtras = { state: { error: error.error } };
              this.router.navigateByUrl('/server-error', navigationExtras);
              this.toastr.error('Error Server', error.status.toString())
              break;
              default:
